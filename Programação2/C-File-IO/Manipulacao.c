...
int main(void){
	...
	//fwrite(pont var , tamanho dela , quantas copias, arquivo);
	}
	while(!feof(alunos)){
		fread(&aluno[contador],sizeof(struct aluno),1,alunos);
		contador++;
	}
	aluno[0].media = aluno[0].nota2+aluno[0].nota1+aluno[0].nota3;
	printf("Nome:%s\nMatricula:%d\nMédia:%d\n",aluno[0].nome,aluno[0].matricula,aluno[0].media/3);
}
